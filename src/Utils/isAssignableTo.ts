...
import { LiteralType, LiteralValue } from "../Type/LiteralType.ts";
...
export function isAssignableTo(
  ...
  if (source instanceof UnionType || source instanceof EnumType) {
    return source.getTypes().every((type) =>
      isAssignableTo(target, type, insideTypes)
    );
  }
  ...
  if (source instanceof IntersectionType) {
    return combineIntersectingTypes(source).some((type) =>
      isAssignableTo(target, type, insideTypes)
    );
  }
  ...
  if (target instanceof ArrayType) {
    ...
    if (source instanceof ArrayType) {
      ...
      return source.getTypes().every((type) =>
        isAssignableTo(targetItemType, type, insideTypes)
      );
    } else {
      return false;
    }
  ...
  if (target instanceof UnionType || target instanceof EnumType) {
    return target.getTypes().some((type) =>
      isAssignableTo(type, source, insideTypes)
    );
  }
  ...
  if (target instanceof IntersectionType) {
    return combineIntersectingTypes(target).every((type) =>
      isAssignableTo(type, source, insideTypes)
    );
  }
  ...
  if (target instanceof ObjectType) {
    ...
    if (
      ...
      (source instanceof NumberType || source instanceof StringType ||
    ...
    if (targetMembers.length === 0) {
      ...
      return !isAssignableTo(
        new UnionType([new UndefinedType(), new NullType()]),
        source,
        insideTypes,
      );
      ...
      return (
        targetMembers.every((targetMember) => {
          ...
          const sourceMember = sourceMembers.find((member) =>
            targetMember.getName() === member.getName()
          );
          return sourceMember == null
            ? inCommon && !targetMember.isRequired()
            : true;
        }) &&
        sourceMembers.every((sourceMember) => {
          const targetMember = targetMembers.find((member) =>
            member.getName() === sourceMember.getName()
          );
    ...
    const isArrayLikeType = source instanceof ArrayType ||
      source instanceof TupleType;
    if (isArrayLikeType) {
