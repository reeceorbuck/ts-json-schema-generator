...
export function getAllOfDefinitionReducer(childTypeFormatter: TypeFormatter) {
  ...
  return (definition: Definition, baseType: BaseType): Definition => {
    ...
    definition.properties = deepMerge(
      other.properties || {},
      definition.properties || {},
    );

    function additionalPropsDefinition(
      props?: boolean | Definition,
    ): props is Definition {
    ...
    if (
      ...
      const addAdditionalProps: (addProps: Definition) => void = (
        addProps: Definition,
      ) => {
        if (addProps !== false) {
          if (addProps.anyOf) {
    ...
    if (other.required) {
      definition.required = uniqueArray(
    ...
    if (
      (other.additionalProperties ||
        other.additionalProperties === undefined) &&
      definition.additionalProperties == false
    ) {
      delete definition.additionalProperties;
    }
