...
import { EnumType, EnumValue } from "../Type/EnumType.ts";
...
import { derefAnnotatedType, derefType } from "../Utils/derefType.ts";
...
export class MappedTypeNodeParser implements SubNodeParser {
  ...
  public createType(
    ...
    const constraintType = this.childNodeParser.createType(
    ...
    if (keyListType instanceof UnionType) {
      ...
      keyListType instanceof StringType || keyListType instanceof SymbolType
    ) {
      // Key type widens to `string`
      const type = this.childNodeParser.createType(node.type!, context);
      const resultType = type === undefined
        ? undefined
        : new ObjectType(id, [], [], type);
  ...
  protected mapKey(
    ...
    const key = derefType(
      this.childNodeParser.createType(
        node.nameType,
        this.createSubContext(node, rawKey, context),
      ),
    );
  ...
  protected getProperties(
    ...
    return keyListType
      ...
      .reduce(
        // @ts-ignore d
        (result: ObjectProperty[], key: LiteralType): ObjectProperty[] => {
  ...
  protected getAdditionalProperties(
    ...
    const key =
      keyListType.getTypes().filter((type) =>
