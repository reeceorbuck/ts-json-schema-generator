...
import { isPublic, isStatic } from "../Utils/modifiers.ts";
...
export class InterfaceAndClassNodeParser implements SubNodeParser {
  ...
  protected getBaseTypes(
    ...
    return node.heritageClauses.reduce(
      (result: BaseType[], baseType) => [
        ...
        ...baseType.types
          .map((expression) =>
            this.childNodeParser.createType(expression, context)
          )
          .filter(notUndefined),
      ],
      [],
  ...
  protected getProperties(
    ...
    const properties =
      (node.members as ts.NodeArray<ts.TypeElement | ts.ClassElement>)
        ...
        .filter((member) =>
          isPublic(member) && !isStatic(member) && member.type &&
          !isNodeHidden(member)
        )
        .map(
          (member) =>
  ...
  protected getAdditionalProperties(
    ...
    const indexSignature = (node.members as ts.NodeArray<ts.NamedDeclaration>)
      .find(ts.isIndexSignatureDeclaration);
    if (!indexSignature) {
  ...
  protected getTypeId(node: ts.Node, context: Context): string {
    const nodeType = ts.isInterfaceDeclaration(node) ? "interface" : "class.ts";
