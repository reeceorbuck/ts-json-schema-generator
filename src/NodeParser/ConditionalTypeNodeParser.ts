...
import { isAssignableTo } from "../Utils/isAssignableTo.ts";
import { narrowType } from "../Utils/narrowType.ts";
...
export class ConditionalTypeNodeParser implements SubNodeParser {
  ...
  public createType(
    ...
    const extendsType = this.childNodeParser.createType(
      node.extendsType,
      context,
    );
    ...
    const trueCheckType = narrowType(
      checkType,
      (type) => isAssignableTo(extendsType, type),
    );
    const falseCheckType = narrowType(
      checkType,
      (type) => !isAssignableTo(extendsType, type),
    );
