...
export class TupleTypeFormatter implements SubTypeFormatter {
  ...
  public getDefinition(type: TupleType): Definition {
    ...
    const requiredElements = subTypes.filter((t) =>
      !(t instanceof OptionalType) && !(t instanceof RestType)
    );
    const optionalElements = subTypes.filter((t) =>
      t instanceof OptionalType
    ) as OptionalType[];
    const restElements = subTypes.filter((t) =>
      t instanceof RestType
    ) as RestType[];
    const restType = restElements.length
      ? restElements[0].getType().getItem()
      : undefined;
    const firstItemType = requiredElements.length > 0
      ? requiredElements[0]
      : optionalElements[0]?.getType();
    ...
    const isUniformArray = firstItemType &&
      requiredElements.every((item) =>
        item.getId() === firstItemType.getId()
      ) &&
      optionalElements.every((item) =>
        item.getType().getId() === firstItemType.getId()
      ) &&
      (restElements.length === 0 ||
        (restElements.length === 1 &&
          restType?.getId() === firstItemType.getId()));
    ...
    if (isUniformArray) {
      return {
        ...
        ...(restType
          ? {}
          : { maxItems: requiredElements.length + optionalElements.length }),
      };
    }
    ...
    const restDefinition = restType
      ? this.childTypeFormatter.getDefinition(restType)
      : undefined;
