...
import { NoRootTypeError } from "./Error/NoRootTypeError.ts";
...
import { localSymbolAtNode, symbolAtNode } from "./Utils/symbolAtNode.ts";
import { notUndefined } from "./Utils/notUndefined.ts";
import { removeUnreachable } from "./Utils/removeUnreachable.ts";
...
export class SchemaGenerator {
  ...
  public createSchemaFromNodes(rootNodes: ts.Node[]): Schema {
    const rootTypes = rootNodes
      ...
      .filter(notUndefined);
    const rootTypeDefinition = rootTypes.length === 1
  ...
  protected appendRootChildDefinitions(
    ...
    const children = this.typeFormatter
      ...
      .filter((child): child is DefinitionType =>
  ...
  protected inspectNode(
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.ArrowFunction:
        allTypes.set(
          `NamedParameters<typeof ${this.getFullName(node, typeChecker)}>`,
          node,
        );
        return;
      default:
